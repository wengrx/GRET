...
class SacreBLEUScorer(object):
    ...
    def _compute_bleu(self, stdin):
        ...
        cmd_bleu = subprocess.Popen(["sacrebleu", "-tok", "intl", "-l", self.lang_pair] + self.sacrebleu_args + ["--score-only",] + self.references,
                                    stdin=stdin,
    ...
    def corpus_bleu_t2t(self, hyp_in):
        print(hyp_in)
        t2tBLEU_cmd = ['t2t-bleu','--translation',hyp_in,'--reference',self.reference_path[:-3]+'.tok'+self.reference_path[-3:]]
        cmd_bleu = subprocess.Popen(t2tBLEU_cmd, stdout=subprocess.PIPE)
        print(cmd_bleu.stdout.read().strip().split())
        print((cmd_bleu.stdout.read().strip().split())[2])
        return float(cmd_bleu.stdout.read().strip().split()[2])
