...
def compute_forward(model,
    ...
    if not eval:
        ...
        with torch.enable_grad():
            ...
            loss = critic(log_probs, labels=y_label, reduce=False, normalization=normalization)
            # loss = model.critic(inputs=log_probs, labels=y_label, reduce=False, normalization=normalization)
        ...
        with torch.no_grad():
            log_probs = model(seqs_x, y_inp)
            loss = critic(log_probs, y_label, reduce=True, normalization=normalization)
        return loss.item()
...
def bleu_validation(uidx,
    ...
    for batch in valid_iter:
        ...
        with torch.no_grad():
            ...
            word_ids = model(src_seq=x,beam_size=beam_size, alpha=alpha, max_steps=max_steps)
    ...
    bleu_v = [bleu_scorer.corpus_bleu(hyp_path),bleu_scorer.corpus_bleu_thumt(hyp_path),bleu_scorer.corpus_bleu_t2t(hyp_path)]
...
def load_pretrained_model(nmt_model, pretrain_path, device, exclude_prefix=None):
    ...
    if pretrain_path != "":
        ...
        for name, params in pretrain_params.items():
            params = params[:,:len(params[0])]+params[:,len(params[0]):-1]
            flag = False
...
def train(FLAGS):
    ...
    nmt_model=torch.nn.DataParallel(nmt_model)
    ...
    while True:
        ...
        for batch in training_iter:
            ...
            if should_trigger_by_steps(global_step=uidx, n_epoch=eidx,
                ...
                model_collections.add_to_collection(key="history_bleus", value=valid_bleu[2])

                best_valid_bleu = float(np.array(model_collections.get_collection("history_bleus")).max())

                summary_writer.add_scalar("Sacre BLEU", valid_bleu[0], uidx)
                summary_writer.add_scalar("THU BLEU", valid_bleu[1],uidx)
                summary_writer.add_scalar("T2T BLEU", valid_bleu[2],uidx)
                summary_writer.add_scalar("best_bleu", best_valid_bleu, uidx)

                # If model get new best valid bleu score
                if valid_bleu[2] >= best_valid_bleu:
                    bad_count = 0
                ...
                INFO("{0} Loss: {1:.2f} Sacre BLEU: {2:.2f} THU BLEU: {3:.2f} T2T BLEU: {4:.2f} lrate: {5:6f} patience: {6} BEST BLEU: {7}".format(
                    uidx, valid_loss, valid_bleu[0], valid_bleu[1], valid_bleu[2], lrate, bad_count, best_valid_bleu
                ))
...
def translate(FLAGS):
    ...
    for batch in valid_iter:
        ...
        with torch.no_grad():
            ...
            word_ids = nmt_model(src_seq=x,beam_size=FLAGS.beam_size, alpha=FLAGS.alpha, max_steps=FLAGS.max_steps)
    ...
    bleu_v = [bleu_scorer.corpus_bleu(outputs[0]),bleu_scorer.corpus_bleu_thumt(outputs[0]),bleu_scorer.corpus_bleu_t2t(outputs[0])]
    INFO('Done. Sacre BLEU Score: {0:.2f}; THU BLEU Score: {1:.2f}; T2T BLEU Score: {2:.2f};'.format(bleu_v[0],bleu_v[1],bleu_v[2]))
